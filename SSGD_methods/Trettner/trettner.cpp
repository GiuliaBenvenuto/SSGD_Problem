...
HalfEdge HEInit(const string& file, vector<int> &sources) {
    ...
    std::unordered_map<int64_t, int> edge_to_idx;
        ...
        // cout << "3. Halfedge to vertex: " << mesh.halfedge_to_vertex.size() << endl;
        // cout << "4. Halfedge to face: " << mesh.halfedge_to_face.size() << endl;
        // cout << "5. Halfedge to next: " << mesh.halfedge_to_next.size() << endl;
        // cout << "6. Halfedge to prev: " << mesh.halfedge_to_prev.size() << endl;
        // cout << "7. Vertex to outgoing halfedge: " << mesh.vertex_to_outgoing_halfedge.size() << endl;
        // cout << "8. Edge lengths: " << mesh.edge_lengths.size() << endl;
        // cout << "9. Average edge length: " << mesh.avg_edge_length << endl;
...
ScalarField distance_field_trettner(const HalfEdge &mesh, const vector<int> &sources) {
    ...
    // add source (currently only works for non-boundary vertices)
    {
        ...
        do
        {
            ...
            if (f != -1)
            {
                ...
                halfedge_to_vertex_distance_sqr[size_t(h0)] = pow2(mesh.edge_lengths[size_t(h0 / 2)]);
                halfedge_to_vertex_distance_sqr[size_t(h1)] = pow2(mesh.edge_lengths[size_t(h2 / 2)]);

                face_to_extra_distance[size_t(f)] = 0.0f;
                face_to_center_distance[size_t(f)] = dist(centroid, mesh.vertex_pos[size_t(sources[0])]);
    ...
    while (!queue_from.empty())
    {
        ...
        for (auto qi = 0u; qi < queue_from.size(); ++qi)
        {
            ...
            auto d2_sqr = halfedge_to_vertex_distance_sqr[size_t(mesh.halfedge_to_prev[size_t(h_source)])];
            assert(d1_sqr >= 0);
            assert(d2_sqr >= 0);
