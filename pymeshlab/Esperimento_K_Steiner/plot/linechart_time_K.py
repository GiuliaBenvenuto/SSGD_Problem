...
for i, txt in enumerate(pre_time):
    plt.text(K[i] - 0.1, pre_time[i], f"{txt:.2f}", fontsize=8, ha='right', va='bottom')
plt.xlabel("K: Number of Rings")
plt.ylabel("Preprocessing Time (seconds)")
plt.title("Preprocessing Time vs. K Value for the Fertility Organic Mesh", fontweight="bold")
plt.grid(True)
plt.savefig("preprocessing_time_plot.png", dpi=300)  # Save the plot with higher resolution
plt.close()
...
for i, txt in enumerate(query_time):
    plt.text(K[i] - 0.1, query_time[i], f"{txt:.5f}", fontsize=8, ha='right', va='bottom')
plt.xlabel("K (Number of Rings)")
plt.ylabel("Query Time (seconds)")
plt.title("Query Time vs. K Value for the Fertility Organic Mesh", fontweight="bold")
plt.grid(True)
plt.savefig("query_time_plot.png", dpi=300)  # Save the plot with higher resolution
plt.close()
