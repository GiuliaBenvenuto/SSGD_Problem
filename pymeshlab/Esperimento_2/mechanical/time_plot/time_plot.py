...
# # Load the CSV file
# mean_times_df = pd.read_csv('../DISTANCES_OK/mean_times_mech_FINAL.csv')  # Make sure to set the correct path to your CSV file
...
# Create a scatter plot for Preprocessing Time with logarithmic scales on both axes
plt.figure(figsize=(10, 6))
sns.scatterplot(data=mean_times_df, x='Mesh Size', y='Preprocessing Time', hue='Method', palette=color_mapping, s=50, marker='o')
plt.xscale('log')  # Set the x-axis to a logarithmic scale
plt.yscale('log')  # Set the y-axis to a logarithmic scale
plt.title('Mesh Size vs Preprocessing Time')
plt.xlabel('Mesh Size (Vertices)')
