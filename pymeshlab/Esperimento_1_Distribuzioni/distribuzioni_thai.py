...
def plot_percentage_errors(percentage_errors, reference_key, mesh_info, plot_name="plot_result"):
    custom_labels = {
        '3_thai_VTP_2.txt': 'thai_1',
        '2_thai_VTP_2.txt': 'thai_0',
        '4_thai_VTP_2.txt': 'thai_2',
        '5_thai_VTP_2.txt': 'thai_3',
        '6_thai_VTP_2.txt': 'thai_4',
        '7_thai_VTP_2.txt': 'thai_5',
        '8_thai_VTP_2.txt': 'thai_6'
    }
    ...
    for i, (key, errors) in enumerate(percentage_errors.items()):
        ...
        if key != reference_key and len(errors) > 0:
            ...
            full_label = f"{label} (μ={mean:.2f}, σ={std:.2f}) [{vertices_label}]"
            
            # Plotting with the default colors sequence
            line, = ax.plot(x_range, p, linewidth=2, label=full_label, color=colors[i % len(colors)])
            lines.append(line)
