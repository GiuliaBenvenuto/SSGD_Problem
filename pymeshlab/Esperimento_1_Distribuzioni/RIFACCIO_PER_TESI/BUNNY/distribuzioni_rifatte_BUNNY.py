...
def plot_percentage_errors(percentage_errors, reference_key, mesh_info, plot_name="plot_result"):
    ...
    for i, (key, errors) in enumerate(percentage_errors.items()):
        ...
        if key != reference_key and len(errors) > 0:
            ...
            trimmed_key = "_".join(key.split("_")[:2])  # Keeps only the first two parts of the filename
    ...
    #              fontweight='bold')
    ax.set_title(f'Bunny Mesh Set Created with Subdivision\n'
                f'Distribution of Percentage Errors for "Bunny" meshes with VTP Method',
                fontweight='bold')
    ...
    save_directory = os.path.join(script_directory, 'bunny_plot')
...
if reference_key in file_data:
    ...
    for filename, errors in percentage_errors.items():
        if filename != reference_key:
            ...
            print()
    plot_percentage_errors(percentage_errors, reference_key, mesh_info, plot_name=f"bunny_{key_string}")
else:
