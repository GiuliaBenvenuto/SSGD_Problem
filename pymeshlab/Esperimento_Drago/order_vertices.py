...
#     # Costruisci un dizionario per una ricerca veloce
#     vertex_to_index = {tuple(v): i for i, v in enumerate(previous_vertices)}

#     map_vector = np.full(len(current_vertices), -1, dtype=int)

#     # Trova gli indici dei vertici in current_mesh che sono già presenti in previous_mesh
#     for i, vertex in enumerate(current_vertices):
#         vertex_tuple = tuple(vertex)
#         if vertex_tuple in vertex_to_index:
#             map_vector[i] = vertex_to_index[vertex_tuple]

#     # Assegna nuovi indici ai vertici non mappati
#     next_index = len(previous_vertices)
#     for i in range(len(map_vector)):
#         if map_vector[i] == -1:
#             map_vector[i] = next_index
#             next_index += 1

#     print("Vertex mapping completed.")
#     return map_vector

# def reorder_mesh(previous_mesh, current_mesh, map_vector):
#     print("Reordering vertices and faces...")
#     # Riordina i vertici di current_mesh
#     reordered_vertices = np.array([current_mesh.vertices[index] for index in map_vector])

#     # Riordina le facce di current_mesh
#     reordered_faces = np.array([[map_vector[vertex_index] for vertex_index in face] for face in current_mesh.faces])

#     print("Reordering completed.")
#     return Mesh(reordered_vertices.tolist(), reordered_faces.tolist())

# def process_meshes(input_folder, output_folder):
#     print("Processing meshes...")
#     # Assumiamo che i file siano nominati M0.obj, M1.obj, ..., M8.obj
#     file_paths = [os.path.join(input_folder, f'M{i}.obj') for i in range(8)]

#     print("Loading all meshes...")
#     # Carica tutte le mesh
#     meshes = [load_mesh(file_path) for file_path in file_paths]

#     # Lista per le mesh riordinate
#     mapped_meshes = [meshes[0]]  # Inizializza con M0

#     # Processa tutte le coppie di mesh
#     for i in range(1, len(meshes)):
#         print(f"Processing mesh {i}...")
#         previous_mesh = mapped_meshes[-1]
#         current_mesh = meshes[i]

#         # Crea il mapping tra previous_mesh e current_mesh
#         map_vector = create_mapping(previous_mesh, current_mesh)

#         # Riordina current_mesh rispetto a previous_mesh
#         reordered_mesh = reorder_mesh(previous_mesh, current_mesh, map_vector)

#         # Aggiungi la mesh riordinata alla lista risultante
#         mapped_meshes.append(reordered_mesh)

#     print("Saving all reordered meshes...")
#     # Salva tutte le mesh riordinate
